...
layui.extend({
  ...
  ,view: 'lib/view' //核心模块
}).define(['setter', 'admin', 'api'], function(exports){
  ...
  ,entryPage = function(fn){
    ...
    //独立页面
    if(isIndPage || pathURL === '/WDHindPage/user/login'){ //此处单独判断登入页，是为了兼容旧版（即未在 config.js 配置 indPage 的情况）
      ...
      if(setter.interceptor){
        ...
        if(!local[setter.request.tokenName]){
          return location.hash = '/WDHindPage/user/login/redirect='+ encodeURIComponent(pathURL); //跳转到登入页
        }
      }
